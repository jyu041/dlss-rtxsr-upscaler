...
The first wrapper invocation then rejected the run because the manifest listed `0x00000000` under `device_removal_results`. That value is the healthy `S_OK` result returned by the native `GetDeviceRemovedReason()` query, not a removal failure. The parser was corrected to record all queried codes separately and reserve `device_removal_results` for failing HRESULT values only.

The required application-level rerun then passed completely on 2026-09-19. It again processed 442 input frames into exactly 884 output frames, generated 432 interpolated frames, recorded no interpolation-disabled frame IDs, reported `device_removal_query_results=["0X00000000"]` with `device_removal_results=[]`, exited the worker with code 0, and reached both `DLSSG_GRID4_CANDIDATE_PASS` and the final evidence marker.

This formally closes the application-level grid4 candidate hardware gate for the tested 640x480/~30-fps source. The result still does not promote grid4 into the normal UI/default worker automatically; broader source-resolution/content validation remains the relevant evidence gap before a default change.
