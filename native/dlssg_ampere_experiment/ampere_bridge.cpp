...
constexpr NvStatus OK=0; constexpr size_t MAX_GPUS=64; thread_local uint32_t depth=0;
Bridge *active=nullptr; std::mutex hook_lock;
...
NvStatus HookedGetArchInfo(NvPhysicalGpuHandle gpu, NvGpuArchInfo *info) {
    return active ? active->GetArchInfo(gpu, info) : -1;
}
