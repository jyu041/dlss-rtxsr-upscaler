...
using NvPhysicalGpuHandle = ::NvPhysicalGpuHandle;
using NvStatus = ::NvAPI_Status;
...
using NvGetAdapterId = NvStatus(__cdecl *)(NvPhysicalGpuHandle, void *);
...
using NvGpuArchInfo = ::NV_GPU_ARCH_INFO;
...
public:
    ...
    bool PrepareArchitecturePreflight(std::string &detail);
    bool RemoveArchitecturePreflight();
    NvStatus QueryNative(NvGpuArchInfo *info) const;
    NvStatus QueryBridged(NvGpuArchInfo *info);
    ...
    uint32_t HookCallCountForTest() const;
