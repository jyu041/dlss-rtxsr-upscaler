...
void printObjectRegion(const std::vector<NV_OF_FLOW_VECTOR>& flow, uint32_t width, uint32_t grid, uint32_t x0, uint32_t y0, uint32_t x1, uint32_t y1, double truthX, double truthY, const char* name)
{
    std::vector<double> errors, magnitudes; for (uint32_t row = y0 / grid; row < y1 / grid; ++row) for (uint32_t col = x0 / grid; col < x1 / grid; ++col) { const auto& v = flow[row * width + col]; double x = v.flowx / 32.0, y = v.flowy / 32.0; if (std::isnan(truthX)) magnitudes.push_back(std::hypot(x, y)); else errors.push_back(std::hypot(x - truthX, y - truthY)); }
    if (std::isnan(truthX)) { std::sort(magnitudes.begin(), magnitudes.end()); auto p = [&](double q) { return magnitudes[static_cast<size_t>(q * (magnitudes.size() - 1))]; }; double mean = 0; for (double value : magnitudes) mean += value; std::cerr << "region=" << name << " count=" << magnitudes.size() << " magnitude_p10_p50_p90_mean=" << p(.1) << "," << p(.5) << "," << p(.9) << "," << mean / magnitudes.size() << "\n"; return; }
    std::sort(errors.begin(), errors.end()); auto p = [&](double q) { return errors[static_cast<size_t>(q * (errors.size() - 1))]; }; double mean = 0; for (double e : errors) mean += e; mean /= errors.size(); std::cerr << "region=" << name << " count=" << errors.size() << " vector_mae_p10_p50_p90_mean=" << p(.1) << "," << p(.5) << "," << p(.9) << "," << mean << "\n";
}
...
void executeKnownTranslation(NV_OF_D3D12_API_FUNCTION_LIST& api, ID3D12Device* device)
{
    ...
    if (g_object) {
        previous.assign(width * height * 4, 28); current.assign(width * height * 4, 28); for (uint32_t y = 0; y < height; ++y) for (uint32_t x = 0; x < width; ++x) { uint8_t background = static_cast<uint8_t>(28 + ((x * 13u + y * 7u) & 31u)); for (uint32_t c = 0; c < 4; ++c) { previous[(y * width + x) * 4 + c] = background; current[(y * width + x) * 4 + c] = background; } }
        for (uint32_t y = 120; y < 200; ++y) for (uint32_t x = 80; x < 160; ++x) { uint8_t value = g_scene == 2 ? 120 : (g_scene == 3 ? static_cast<uint8_t>(120 + ((x / 8u) & 1u) * 100u) : static_cast<uint8_t>(40 + ((x * 29u + y * 17u) & 191u))); for (uint32_t c = 0; c < 4; ++c) previous[(y * width + x) * 4 + c] = value; }
        for (uint32_t y = 120; y < 200; ++y) for (uint32_t x = 88; x < 168; ++x) { uint32_t sourceX = x - 8; uint8_t value = g_scene == 2 ? 120 : (g_scene == 3 ? static_cast<uint8_t>(120 + ((sourceX / 8u) & 1u) * 100u) : static_cast<uint8_t>(40 + ((sourceX * 29u + y * 17u) & 191u))); for (uint32_t c = 0; c < 4; ++c) current[(y * width + x) * 4 + c] = value; }
    }
    ...
    print("input_current_reference_previous", forward[(height / 2 / g_grid) * outputWidth + 120 / g_grid]); if (g_object) { printObjectRegion(forward, outputWidth, g_grid, 100, 140, 150, 180, -8, 0, "object_interior"); printObjectRegion(forward, outputWidth, g_grid, 152, 140, 160, 180, -8, 0, "leading_valid"); printObjectRegion(forward, outputWidth, g_grid, 96, 140, 104, 180, -8, 0, "near_trailing_valid"); printObjectRegion(forward, outputWidth, g_grid, 300, 100, 500, 300, 0, 0, "static_background"); printObjectRegion(forward, outputWidth, g_grid, 80, 120, 88, 200, NAN, NAN, "disoccluded_ambiguous"); printObjectRegion(forward, outputWidth, g_grid, 160, 120, 168, 200, NAN, NAN, "occlusion_ambiguous"); } else printRegionStats(forward, outputWidth, g_grid);
...
int wmain()
{
    char option[32]{}; if (GetEnvironmentVariableA("NVOF_NO_COST", option, sizeof(option))) g_enableCost = false; if (GetEnvironmentVariableA("NVOF_R8", option, sizeof(option))) g_r8 = true; if (GetEnvironmentVariableA("NVOF_REVERSE", option, sizeof(option))) g_reverse = true; if (GetEnvironmentVariableA("NVOF_OBJECT", option, sizeof(option))) { g_object = true; g_r8 = false; } if (GetEnvironmentVariableA("NVOF_SCENE", option, sizeof(option))) { g_object = true; g_r8 = false; g_scene = std::stoi(option); } if (GetEnvironmentVariableA("NVOF_FAST", option, sizeof(option))) g_perf = NV_OF_PERF_LEVEL_FAST; if (GetEnvironmentVariableA("NVOF_SLOW", option, sizeof(option))) g_perf = NV_OF_PERF_LEVEL_SLOW; if (GetEnvironmentVariableA("NVOF_BOTH", option, sizeof(option))) g_direction = NV_OF_PRED_DIRECTION_BOTH; if (GetEnvironmentVariableA("NVOF_GRID", option, sizeof(option))) g_grid = static_cast<uint32_t>(std::stoul(option)); if (GetEnvironmentVariableA("NVOF_RING", option, sizeof(option))) g_ringDepth = static_cast<uint32_t>(std::stoul(option));
