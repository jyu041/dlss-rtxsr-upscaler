...
class DLSSGBackend(Backend):
    ...
    def status(self) -> BackendStatus:
        ...
        if config.runtime_profile == "candidate-0.3.1":
            expected = current_attestation(runtime_path=config.community_runtime, ini_path=config.community_runtime.with_name("dlssg_sm86.ini"), worker_path=config.worker)
