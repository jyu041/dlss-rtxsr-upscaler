...
def progress_html(progress: JobProgress) -> str:
    ...
    total = "?" if progress.frames_total is None else str(progress.frames_total)
    eta = "Estimating..." if progress.eta_seconds is None and progress.state not in {"COMPLETED", "CANCELLED", "FAILED"} else format_duration(progress.eta_seconds or 0)
    ...
    return (
        ...
        f'<div class="progress-meta"><span>Frames: {progress.frames_done} / {total}</span><span>FPS: {fps:.2f}</span><span>Elapsed: {format_duration(progress.elapsed_seconds)}</span><span>ETA: {escape(eta)}</span></div>'
