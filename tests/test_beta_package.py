...
def test_legacy_beta2_builder_has_explicit_commit_hash_gate_and_external_boundaries():
    script = (Path(__file__).parents[1] / "tools" / "history" / "build_v0_1_0_beta_2_package.ps1").read_text(encoding="utf-8")
...
def test_legacy_beta2_builder_has_exact_identity_gates_and_clean_package_policy():
    script = (Path(__file__).parents[1] / "tools" / "history" / "build_v0_1_0_beta_2_package.ps1").read_text(encoding="utf-8")
...
def test_legacy_beta2_builder_sources_binary_notice_from_docs_legal_only():
    script = (Path(__file__).parents[1] / "tools" / "history" / "build_v0_1_0_beta_2_package.ps1").read_text(encoding="utf-8")
...
def test_legacy_beta2_builder_is_explicitly_historical():
    script = (
        Path(__file__).parents[1]
        / "tools"
        / "history"
        / "build_v0_1_0_beta_2_package.ps1"
