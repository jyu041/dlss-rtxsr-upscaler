...
def _pin_fake_runtime(monkeypatch, root: Path):
    pinned = {}
    for name in v10.V10_FILES:
        path = root / name
        pinned[name] = {
            "size_bytes": path.stat().st_size,
            "sha256": v10._sha256(path),
            "authenticode": "NotSigned",
        }
    monkeypatch.setattr(v10, "V10_EXPECTED_FILES", pinned)
...
def test_v10_static_boundary_uses_pinned_hashes_by_default(monkeypatch, tmp_path):
    runtime = tmp_path / "v10"
    _write_runtime(runtime)
    _pin_fake_runtime(monkeypatch, runtime)
    monkeypatch.setattr(
        v10,
        "inspect_pe",
