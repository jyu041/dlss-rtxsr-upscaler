...
def test_independent_unicode_user_presets_and_last_used(tmp_path, monkeypatch):
    ...
    sr = {"mode": "Quality", "model_preset": "K"}
    user_presets.save_user_preset("dlss_sr", "我的 SR", sr)
    assert user_presets.get_user_preset("rtx_vsr", "我的 RTX") == rtx
    assert user_presets.get_user_preset("dlss5", "我的 DLSS") == dlss
    assert user_presets.get_user_preset("dlss_sr", "我的 SR") == sr
    ...
    user_presets.save_last_used("dlss_sr", sr)
    assert user_presets.load_last_used()["dlss_sr"] == sr
