...
def _copy_external_runtime(source: Path | None, destination: Path, required: tuple[str, ...], label: str, relative_prefix: str) -> list[dict[str, object]]:
    ...
    return [{"path": f"{relative_prefix}/{path.relative_to(destination).as_posix()}", "sha256": sha256(path), "size_bytes": path.stat().st_size} for path in sorted(destination.rglob("*")) if path.is_file() and "__pycache__" not in path.parts and path.suffix.lower() != ".pyc" and not (path.parent.name.endswith(".dist-info") and path.name == "RECORD") and not (path.parent.name == "bin" and "site-packages" in path.parent.parent.parts)]
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--lock", type=Path, help="artifact lock; assembles PythonCore and FFmpeg from verified cache inputs")
    parser.add_argument("--cache-dir", type=Path, help="verified artifact cache used with --lock")
    args = parser.parse_args(argv)
    try:
        if args.lock:
            if args.python_runtime or args.ffmpeg_runtime: raise RuntimeError("--lock cannot be combined with preassembled runtime inputs")
            cache = (args.cache_dir or args.source_root / '.cache' / 'portable').resolve()
            ...
            with tempfile.TemporaryDirectory(prefix='nve-assemble-') as td:
                assembled = Path(td)
                wheel_cache = cache / 'wheels' if (cache / 'wheels').is_dir() and len(list((cache / 'wheels').glob('*.whl'))) >= len(json.loads(args.lock.read_text(encoding='utf-8'))['artifacts']) else cache / 'phase5i-wheels'
                ...
                print(json.dumps(build(args.output, args.source_root.resolve(), assembled / 'runtime' / 'python', assembled / 'runtime' / 'tools' / 'ffmpeg', py_notice, args.source_root / 'docs' / 'legal' / 'BINARY_DISTRIBUTION_NOTICES.md', (args.toolchain or args.source_root / 'tools' / 'portable_toolchain.json').resolve()), indent=2))
        else:
