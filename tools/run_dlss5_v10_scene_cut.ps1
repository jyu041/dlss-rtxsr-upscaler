...
if (-not $Execute) {
    throw "Bounded DLSS5 v10 scene-cut execution is blocked unless -Execute is supplied explicitly."
}
...
if (-not $Output) {
    $Output = Join-Path $root 'runtime\audit\dlss5-v10-scene-cut-hardware.json'
}
...
$validate = Join-Path $PSScriptRoot 'validate_dlss5_v10_scene_cut.py'
...
Write-Host "DLSS5_V10_SCENE_CUT_ARCHIVE_START archive=$archivePath"
& $audit -Archive $archivePath
if ($LASTEXITCODE -ne 0) {
    throw "DLSS5 v10 archive download/static audit failed: $LASTEXITCODE"
}

Write-Host "DLSS5_V10_SCENE_CUT_PREFLIGHT_START archive=$archivePath"
& $python $prepare --archive $archivePath
if ($LASTEXITCODE -ne 0) {
    throw "DLSS5 v10 candidate preparation/Defender preflight failed: $LASTEXITCODE"
}

Write-Host "DLSS5_V10_SCENE_CUT_START input=$inputResolved start_frame=$StartFrame gpu=$GpuOrdinal"
& $python $validate `
    --input $inputResolved `
    --runtime $runtime `
    --preflight $preflight `
    ...
    --execute `
    --ack BOUNDED_256_SCENE_CUT_32
if ($LASTEXITCODE -ne 0) {
    throw "DLSS5 v10 scene-cut/reset hardware gate failed: $LASTEXITCODE"
}

Write-Host "DLSS5_V10_SCENE_CUT_PASS evidence=$outputPath"
