...
if (-not $Execute) {
    throw "Bounded DLSS5 v10 temporal execution is blocked unless -Execute is supplied explicitly."
}
...
if (-not $Output) {
    $Output = Join-Path $root 'runtime\audit\dlss5-v10-temporal-hardware.json'
}
...
$validate = Join-Path $PSScriptRoot 'validate_dlss5_v10_temporal.py'
...
& $python $validate `
    ...
    --execute `
    --ack BOUNDED_256_THREE_FRAME
if ($LASTEXITCODE -ne 0) {
    throw "DLSS5 v10 bounded temporal hardware gate failed: $LASTEXITCODE"
}

Write-Host "DLSS5_V10_TEMPORAL_PASS evidence=$outputPath"
